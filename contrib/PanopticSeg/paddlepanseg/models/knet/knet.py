...
class KNet(TwoStageDetector):
    ...
    def forward_train(self,
        ...
        for i, gt_mask in enumerate(gt_masks):
            ...
            mask_tensor = paddle.to_tensor(gt_mask.masks)
            if gt_mask.width != pad_W or gt_mask.height != pad_H:
                pad_wh = (0, pad_W - gt_mask.width, 0, pad_H - gt_mask.height)
                mask_tensor = F.pad(mask_tensor[:, None], pad_wh, value=0, data_format="NCHW").squeeze(1)
            ...
            if mask_tensor.shape[0] == 0:
                gt_masks_tensor.append(
                    paddle.zeros(
                        (mask_tensor.shape[0], assign_H, assign_W)).cast(mask_tensor.dtype)
                    )
            else:
                gt_masks_tensor.append(
                    F.interpolate(
                        mask_tensor[None].cast("float32"), (assign_H, assign_W),
                        mode='bilinear',
                        align_corners=False)[0])
...
if __name__ == "__main__":
    ...
    gt_masks=[
        Dict(
            areas = np.array([ 36364, 120105]),
            height = 736,
            masks = np.random.randint(0, 2, (2, 736, 960)),
            width = 960),
        Dict(
            areas = np.array([206318]),
            height = 800,
            masks = np.random.randint(0, 2, (1, 800, 1344)),
            width = 1344),
    ]

    gt_semantic_seg = paddle.randint(0, 89, [2, 1, 800, 1344],dtype="int32")


    knet.forward_train(img, img_metas, gt_bboxes, gt_labels,
                    gt_bboxes_ignore, gt_masks, proposals, gt_semantic_seg)