...
class BaseRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if bbox_head is not None: # False
...
class KernelIterHead(BaseRoIHead):
    ...
    def init_mask_head(self, mask_roi_extractor, mask_head):
        ...
        if not isinstance(mask_head, list): # False
            mask_head = [mask_head for _ in range(self.num_stages)]
        assert len(mask_head) == self.num_stages
        for head in mask_head:
            if "type" in head:
                head.pop("type")
            head_instance = KernelUpdateHead(**head)
            # head_instance = build_head(head)
            self.mask_head.append(head_instance)
        if self.recursive:
            for i in range(self.num_stages):
    ...
    def forward_train(self,
        ...
        if self.mask_head[0].mask_upsample_stride > 1:
            prev_mask_preds = F.interpolate( # 上采样
                mask_preds.detach(),
        ...
        if cls_score is not None: # False
    ...
    def forward_dummy(self, x, proposal_boxes, proposal_feats, img_metas):
        ...
        all_stage_mask_results = []
        num_imgs = len(img_metas)
        num_proposals = proposal_feats.shape[1]
        C, H, W = x.shape[-3:]
        mask_preds = proposal_feats.bmm(x.reshape([num_imgs, C, -1])).reshape(
    ...
    def get_panoptic(self, cls_scores, mask_preds, test_cfg, img_meta):
        ...
        total_scores = paddle.concat([thing_scores, stuff_scores], dim=0)
        total_labels = paddle.concat([thing_labels, stuff_labels], dim=0)
    ...
    def merge_stuff_thing(self,
        ...
        cur_prob_masks = total_scores.reshape([-1, 1, 1]) * total_masks
        cur_mask_ids = cur_prob_masks.argmax(0)

        # sort instance outputs by scores
        sorted_inds = paddle.argsort(-total_scores)
...
if __name__ == "__main__":
    ...
    head = KernelIterHead(**roi_head)
    
    
    x = paddle.rand([2, 256, 92, 124])
    proposal_feats = paddle.rand([2, 153, 256, 1, 1])
    mask_preds = paddle.rand([2, 153, 92, 124])
    cls_score = None
    img_metas = [{}, {}]
    gt_masks = [paddle.randint(0, 1, [6, 184, 248]).cast("float32"), 
                paddle.randint(0, 1, [4, 184, 248]).cast("float32")]
    gt_labels = [
        paddle.to_tensor([41, 44, 44, 45, 48, 60]),
        paddle.to_tensor([ 0, 41, 62, 66])
    ]
    ...
    gt_bboxes=None,
    gt_sem_seg=[
        (paddle.rand([2, 184, 248]) > 0.5).cast("float32"),
        (paddle.rand([3, 184, 248]) > 0.5).cast("float32")]
    gt_sem_cls=[
        paddle.to_tensor([121, 131]),
        paddle.to_tensor([116, 121, 131])
    ]
