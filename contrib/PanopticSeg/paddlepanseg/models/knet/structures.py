...
import paddle
from .cv_base import rescale_size, imrescale, imresize, impad, imflip# , imtranslate, imshear, imrotate
...
class BitmapMasks(BaseInstanceMasks):
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if isinstance(self.masks, np.ndarray):
        ...
        elif isinstance(self.masks, paddle.Tensor):
            ...
            if len(self.masks) == 0:
                ...
                padded_masks = np.stack([
                    impad(mask, shape=out_shape, pad_val=pad_val)
                    for mask in self.masks.cpu().numpy()
                ])
            
            return_mask = BitmapMasks(padded_masks, *out_shape)
            return_mask.masks = paddle.to_tensor(return_mask.masks)
    ...
    def to_tensor(self, dtype, device=None):
        ...
        return paddle.to_tensor(self.masks, dtype=dtype)
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def to_tensor(self, dtype, device):
        ...
        return paddle.to_tensor(ndarray_masks, dtype=dtype)
