...
from .cv_base import imrescale, imresize, is_list_of, imfrombytes, imresize, imnormalize, \
    impad, impad_to_multiple, imflip

from .structures import PolygonMasks, BitmapMasks
...
__all__ = [
    "LoadImageFromFile", "LoadAnnotations", "LoadPanopticAnnotations", "Resize", "RandomFlip",
...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if 'img' in results:
            ...
            results['img'] = paddle.to_tensor(img)
        for key in ['proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels']:
            if key not in results:
                continue
            # results[key] = DC(to_tensor(results[key]))
            results[key] = paddle.to_tensor(results[key])
        if 'gt_masks' in results:
            # results['gt_masks'] = DC(results['gt_masks'], cpu_only=True)
            results['gt_masks'].masks = paddle.to_tensor(results['gt_masks'].masks)  # TODO
            
        if 'gt_semantic_seg' in results:
            # results['gt_semantic_seg'] = DC(
            #     to_tensor(results['gt_semantic_seg'][None, ...]), stack=True)
            results['gt_semantic_seg'] = paddle.to_tensor(results['gt_semantic_seg'][None, ...])
